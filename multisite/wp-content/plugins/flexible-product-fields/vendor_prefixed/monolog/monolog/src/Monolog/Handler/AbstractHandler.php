 ...
 */
abstract class AbstractHandler implements \VendorFPF\Monolog\Handler\HandlerInterface, \VendorFPF\Monolog\ResettableInterface
{
    protected $level = \VendorFPF\Monolog\Logger::DEBUG;
     ...
     */
    public function __construct($level = \VendorFPF\Monolog\Logger::DEBUG, $bubble = \true)
    {
        $this->setLevel($level);
        $this->bubble = $bubble;
    }
    ...
    {
        $this->level = \VendorFPF\Monolog\Logger::toMonologLevel($level);
        return $this;
    }
    /**
    ...
    public function reset()
    {
        foreach ($this->processors as $processor) {
            if ($processor instanceof \VendorFPF\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
