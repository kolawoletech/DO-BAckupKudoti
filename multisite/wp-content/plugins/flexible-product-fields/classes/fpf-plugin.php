...
class Flexible_Product_Fields_Plugin extends VendorFPF\WPDesk\PluginBuilder\Plugin\AbstractPlugin implements VendorFPF\WPDesk\PluginBuilder\Plugin\HookableCollection {
	 ...
	 * @var string
	 */
	private $scripts_version = FLEXIBLE_PRODUCT_FIELDS_VERSION . '.50';
	...
	/**
	 * Script loader tag filter.
	 *
	 * @param string $tag Tag.
	 * @param string $handle Handel.
	...
	public function script_loader_tag( $tag, $handle, $src ) {
		if ( 'fpf_admin' === $handle ) {
			...
			$tag = str_replace( "<script src=", "<script type='text/babel' src=", $tag ); // phpcs:ignore
		}
