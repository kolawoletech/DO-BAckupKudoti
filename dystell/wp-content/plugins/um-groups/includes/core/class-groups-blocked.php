...
class Groups_Blocked extends \um\core\Member_Directory {
	...
	/**
	 * Change where query to show only blocked members
	 *
	 * @param $query
	 *
	 * @return mixed
	 */
	function only_blocked( $query ) {
		global $wpdb;
	...
	function ajax_get_members() {
		...
		$directory_data = apply_filters( 'um_group_blocked_directory_data', array(
			'header'         => '',
			'header_single'  => '',
			'show_tagline'   => false,
			'show_userinfo'  => false,
		...
		add_action( 'pre_user_query', array( &$this, 'only_blocked' ) );

		$user_query = new \WP_User_Query( $this->query_args );

		remove_action( 'pre_user_query', array( &$this, 'only_blocked' ) );
	...
	function get_group_users_data( $user_ids, $group_id ) {
		...
		$users_data = $wpdb->get_results( $wpdb->prepare(
			"SELECT `role`, `date_joined`, `user_id1` AS ID
			FROM {$wpdb->prefix}um_groups_members
			WHERE user_id1 IN('" . implode( "','", $user_ids ) . "') AND 
				  group_id = %d",
			$group_id
		), ARRAY_A );
		...
		return apply_filters( 'um_get_group_users_data_blocked', $data, $user_ids, $group_id );
	}
	...
	function prepare_users_response( $user_ids, $directory_data, $group_id ) {
		...
		foreach ( $user_ids as $i => $user_id ) {
			...
			$users[ $i ]['date'] = sprintf( __( 'Joined %s ago', 'um-groups' ), human_time_diff( strtotime( $user_data['date_joined'] ), current_time( 'timestamp' ) ) );

			$joined = ! empty( $user_data['date_joined'] );

			ob_start();
			do_action( 'um_groups_users_list_after_details', $user_id, $group_id, $joined );
