...
if (!class_exists('WPFA_Table_Columns_Manager')) {
	...
	class WPFA_Table_Columns_Manager {
		...
		function remove_columns($original_columns) {
			...
			$all_columns = $wpdb->get_col("SELECT meta_value FROM $wpdb->postmeta pm LEFT JOIN $wpdb->posts p ON p.ID = pm.post_id WHERE p.post_status = 'publish' AND  meta_key = 'vgfa_disabled_columns' AND meta_value LIKE '%\"" . esc_sql($post_type) . "\"%' ");
