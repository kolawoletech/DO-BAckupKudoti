...
if (!class_exists('WPFA_Show_Own_Posts')) {
	...
	class WPFA_Show_Own_Posts {
		...
		function filter_posts_query($wp_query) {
			...
			$restricted_post_types = array_map('trim', explode(',', $vgfa->get_settings('restrict_post_types_by_author', '')));
			if (in_array($post_type, $restricted_post_types, true)) {
				$is_post_type_activated = true;
			} else {
				$is_post_type_activated = (bool) $wpdb->get_var("SELECT COUNT(meta_value) FROM $wpdb->postmeta pm LEFT JOIN $wpdb->posts p ON p.ID = pm.post_id WHERE p.post_status = 'publish' AND meta_key = 'vgfa_show_own_posts' AND meta_value LIKE '%\"" . esc_sql($post_type) . "\"%' LIMIT 1");
			}
