<?php
/*
  Plugin Name: WP Frontend Admin (Premium)
  ...
  @fs_premium_only /inc/global-page.php
  License:
...
if (dapof_fs()->can_use_premium_code__premium_only()) {
	require_once __DIR__ . '/inc/global-page.php';
}
...
if (!class_exists('VG_Admin_To_Frontend')) {
	...
	class VG_Admin_To_Frontend {
		...
		function get_settings($key = null, $default = null) {
			...
			return apply_filters('vg_frontend_admin/settings', $out, $key, $default);
		}
		...
		function init() {
			...
			add_action('transition_post_status', array($this, 'redirect_to_new_after_publish'), 10, 3);

			if (!empty($_POST)) {
				add_action('wp_authenticate', array($this, 'redirect_to_login_page_after_empty_credentials'), 10, 2);
		...
		function get_login_url($default = null) {
			...
			if (is_multisite()) {
				foreach (get_sites(array('order' => 'DESC')) as $site) {
		...
		function prepare_loose_url($url) {
			$url = remove_query_arg(array('post', 'token', '_wpnonce'), $url);
		...
		function enforce_frontend_dashboard() {
			...
			$redirect_to = add_query_arg(array('wpfa_frontend_url' => 1), $redirect_to);
			?>
			<script>
				...
				if (window.parent.location.href === window.location.href && window.location.href.indexOf('http') > -1) {
					window.parent.location.href = <?php echo json_encode(esc_url($redirect_to)); ?>;
				}
			</script>
			<?php
		}
		...
		function hide_system_pages__premium_only($wp_query) {
			...
			if (empty($this->get_settings('hide_system_pages')) || !is_admin() || is_network_admin() || $wp_query->query['post_type'] !== 'page' || !$wp_query->is_main_query() || $this->is_master_user()) {
				return $wp_query;
			}
		...
		function save_quick_settings() {
			...
			if (!empty($update)) {
				$wpdb->update($wpdb->posts, $update, array('ID' => $post_id), '%s', '%d');
			}
		...
		function add_direct_link_menu($wp_admin_bar) {
			...
			$args = array(
				...
				'meta' => array('class' => 'vgca-direct-frontend-link')
			);
		...
		function register_menu_page() {
			add_menu_page(
					VG_Admin_To_Frontend::$name, VG_Admin_To_Frontend::$name, 'manage_options', 'wpatof_welcome_page', array($this->vg_plugin_sdk, 'render_welcome_page'), plugins_url('/assets/imgs/wp-admin-icon.png', __FILE__)
			);
		}
		...
		function identify_source_id() {
			...
			if (strpos($referer, 'vgfa_source=') !== false && empty($_GET['vgfa_source'])) {
				...
				setcookie('vgfa_source', $_GET['vgfa_source'], null, '/');
		...
		function render_admin_css($source_id = null) {
			...
			<?php if (!empty($this->get_settings('disable_all_admin_notices'))) { ?>
...
// Hide elements selected on quick-settings
			...
			<script>
				var vgfaCustomCss = <?php echo json_encode($custom_css); ?>;
				var vgfaWpAdminBase = <?php echo json_encode(admin_url()); ?>;
			</script>
			<?php
		}
		...
		function cleanup_admin_page_for_frontend() {
			...
			?>
			<script>
				var vgfaRequiredRoles = <?php echo json_encode($required_capability_html); ?>;
			...
			<?php
			if (dapof_fs()->can_use_premium_code__premium_only()) {
				$custom_js = $this->get_settings('admin_view_js', '');
			...
			wp_enqueue_script('vg-frontend-admin-init', plugins_url('/assets/js/backend.js', VG_Admin_To_Frontend::$file), array('jquery'), filemtime(VG_Admin_To_Frontend::$dir . '/assets/js/backend.js'));
		}
		...
		function get_login_form($atts = array(), $content = '') {
			...
			if (is_user_logged_in()) {
				...
				if ($redirect_to) {
					?>
					<script>window.location.href = <?php echo json_encode(esc_url($redirect_to)); ?>;</script>
					<?php
				}
				return ob_get_clean();
			}
			...
			$login_form = wp_login_form(array('echo' => false, 'redirect' => $redirect_to));
		...
		function get_edit_link($atts = array(), $content = '') {
			...
			ob_start();
			edit_post_link(__('Edit', VG_Admin_To_Frontend::$textname), '', '', $post_id);
			$out = ob_get_clean();
			return $out;
		}
		...
		function get_admin_page_for_frontend($atts = array(), $content = '') {
			...
			wp_enqueue_script('vg-frontend-admin-init', plugins_url('/assets/js/frontend.js', VG_Admin_To_Frontend::$file), array('jquery'), filemtime(VG_Admin_To_Frontend::$dir . '/assets/js/frontend.js'));
			...
			wp_localize_script('vg-frontend-admin-init', 'vgfa_data', apply_filters('vg_admin_to_frontend/frontend/js_data', array(
				...
				'fullscreen_pages_keywords' => array_values(array_filter(array_merge($full_screen_keywords, array('action=elementor', 'page=formidable-settings', 'page=formidable&fr', 'page=formidable-styles', 'page=wpforms-builder', 'customize.php'))))
			)));
			return ob_get_clean();
		}
