...
if (empty($login_page_url)) {
	...
	if (dapof_fs()->can_use_premium_code__premium_only()) {
		if (!is_user_logged_in() && !empty($this->get_settings('demo_user')) && !empty($this->get_settings('demo_password'))) {
			?>

			<script>
				if (window.location.href.indexOf('noautologin') < 0) {
					document.getElementById('user_login').value = <?php echo json_encode($this->get_settings('demo_user')); ?>;
					document.getElementById('user_pass').value = <?php echo json_encode($this->get_settings('demo_password')); ?>;
	...
	<script>
		window.location.href = <?php echo json_encode(esc_url($login_page_url)); ?>;
	</script>
	<?php
	die();
}