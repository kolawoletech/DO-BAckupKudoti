...
if (!class_exists('WPFA_Show_Own_Posts')) {
	...
	class WPFA_Show_Own_Posts {
		...
		function filter_posts_query($wp_query) {
			...
			if (!is_user_logged_in() || !current_user_can('edit_posts') || wp_doing_ajax() || wp_doing_cron() || $vgfa->is_master_user()) {
				return;
			}
			...
			$is_post_type_activated = (bool) $wpdb->get_var("SELECT COUNT(meta_value) FROM $wpdb->postmeta WHERE meta_key = 'vgfa_show_own_posts' AND meta_value LIKE '%\"" . esc_sql($post_type) . "\"%' LIMIT 1");
